...
void main() {
  ...
  test('M1b: StafirModeToggleExt.next cycles letters → match → cvc → '
  ...
  testWidgets('M3 (Phase 12 UI-02): icon is the SAME across all modes — the '
    ...
    expect(
      ...
      1,
      reason:
          'all 4 mode icons must be identical — '
  ...
  testWidgets('M4: hold less than 3s does NOT toggle', (tester) async {
    ...
    await tester.pumpWidget(
      _host(mode: StafirMode.letters, onToggle: () => callCount++),
    );
    await tester.pump();
  ...
  testWidgets('M9: hold ring appears only while holding', (tester) async {
    await tester.pumpWidget(_host(mode: StafirMode.letters, onToggle: () {}));
    await tester.pump();
    expect(find.byKey(const Key('stafir-mode-toggle-hold-ring')), findsNothing);
    final gesture = await tester.startGesture(
      tester.getCenter(find.byType(StafirModeToggle)),
    );
    await tester.pump(const Duration(milliseconds: 100));
